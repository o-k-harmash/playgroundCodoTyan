import Filter from "@/assets/filter.svg?react"
import GlyphRight from "@/assets/glyph__right.svg?react"
import GlyphLeft from "@/assets/glyph__left.svg?react"
import useTopicBrowseViewModel from "@/viewModels/TopicBrowse/TopicBrowseViewModel"

/**
 * TopicsBrowsePageView
 * --------------------
 * A simple UI component to display a list of topics and tags.
 * Uses a view model (MVVM pattern) to manage state and actions.
 *
 * Features:
 * - Displays tags with clickable selection
 * - Shows a list of topic items with title, description, and associated tags
 * - Shows pagination info (offset, limit, total)
 */
export default function TopicBrowsePageView() {
  const {
    tags,
    items,
    pagination,
    status,
    error,
    isFilterExpanded,
    updateFilterExpandedUseCase,
    updatePageUseCase,
    updateTagUseCase,
  } = useTopicBrowseViewModel()

  if (status === "idle") {
    return (
      <div data-visible="true" className="page-loader">
        <div className="page-loader__spinner"></div>
      </div>
    )
  }

  // Error state: display error message
  if (error) {
    //must to be catched by router dom
    return (
      <div className="py-8 text-center text-red-500">
        Something went wrong, please reload the page. {error}
      </div>
    )
  }

  return (
    <>
      <div data-visible={status === "loading"} className="page-loader">
        <div className="page-loader__spinner"></div>
      </div>

      {/* Tags filter section */}
      <div className="container-sm mt-8">
        <div className="flex flex-col gap-4">
          <button
            type="button"
            className="btn btn--size-sm btn-ghost"
            aria-expanded={isFilterExpanded}
            aria-controls="filter-panel"
            onClick={updateFilterExpandedUseCase}
          >
            <Filter />
          </button>

          <div
            id="filter-panel"
            data-open={isFilterExpanded}
            className="flex max-h-0 -translate-y-1 flex-wrap gap-2
              overflow-hidden opacity-0
              transition-[max-height,opacity,transform] duration-200 ease-[ease]
              data-[open=true]:max-h-[200px] data-[open=true]:translate-y-0
              data-[open=true]:opacity-100"
          >
            {tags.map((tag) => (
              <button
                key={tag.id}
                type="button"
                onClick={() => updateTagUseCase(tag.id)}
                className={`tag ${tag.selected ? "tag-selected" : ""}`}
              >
                {tag.id}
              </button>
            ))}
          </div>
        </div>
      </div>

      {/* Topics list section */}
      <div className="mx-auto mt-8 w-[412px] px-4">
        <ul className="flex w-full max-w-2xl flex-col gap-8">
          {items.map((item) => (
            <li
              key={item.id}
              className="flex flex-col gap-4 rounded-md border
                border-(--color--border) p-4"
            >
              <h3>{item.title}</h3>

              {/* Display tags for each topic */}
              <div className="flex flex-wrap gap-1">
                {item.tags.map((t) => (
                  <span key={t} className="tag">
                    {t}
                  </span>
                ))}
              </div>

              <p className="text-justify">{item.description}</p>
            </li>
          ))}
        </ul>
      </div>

      {/* Pagination / info section */}
      <div
        className="mx-auto mt-8 flex w-[412px] flex-col items-center
          justify-center gap-4 px-4 text-gray-600"
      >
        <div className="flex gap-2">
          <button
            disabled={pagination.prevChank === null}
            onClick={() => updatePageUseCase(pagination.prevChank!)}
          >
            <GlyphLeft className="h-4 w-4"></GlyphLeft>
          </button>

          {pagination.chanck.map((c) => (
            <>
              {c ? (
                <button
                  type="button"
                  key={c.id}
                  disabled={c.disabled}
                  onClick={() => updatePageUseCase(c.id)} // Toggle tag selection
                  className={`px-2 py-3
                    ${c.disabled ? "bg-gray-600 text-white" : "border border-gray-300 text-gray-800"}`}
                >
                  {c.name}
                </button>
              ) : (
                <span className="px-2 py-3">{"..."}</span>
              )}
            </>
          ))}
          <button
            disabled={pagination.nextChanck === null}
            onClick={() => updatePageUseCase(pagination.nextChanck!)}
          >
            <GlyphRight className="h-4 w-4"></GlyphRight>
          </button>
        </div>
      </div>
    </>
  )
}
